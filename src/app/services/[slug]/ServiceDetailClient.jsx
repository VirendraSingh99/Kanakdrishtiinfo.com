...
import { CITIES } from "../citydata/citiesData";
...
export default function ServiceDetailClient({ service, city = null }) {
  ...
  return (
    <main className="w-full bg-white text-slate-900 overflow-x-hidden">
      ...
      <section className="relative w-full min-h-[60vh] sm:min-h-[65vh] lg:min-h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white py-12 sm:py-10 lg:py-10">
          {/* City badge — sirf city page par */}
          {city && (
          ...
          <h1 className="text-xl md:text-4xl font-extrabold leading-relaxed mb-5 tracking-tight">
            {city ? `${service.title} in ${city.name}` : service.title}
          </h1>
          <p className="text-base sm:text-lg md:text-xl text-gray-300 leading-relaxed max-w-4xl mx-auto mb-8 px-2">
            {city
              ? `Looking for professional ${service.title} services in ${city.name}? ${service.heroDescription}`
      ...
      {/* ── CITY INTRO BOX — sirf city page par ── */}
      {city && (
        <section className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="bg-[#E8F7FC] border border-[#49BCE3]/30 rounded-2xl p-6 md:p-10">
            ...
            <p className="text-gray-600 text-base md:text-lg leading-relaxed">
              ...
              market, competition, and customer behavior — giving your business
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center mb-14 md:mb-20">
            ...
            >
              <img
                ...
                alt={service.title}
                className="rounded-2xl shadow-xl w-full h-auto min-h-[280px] sm:min-h-[320px] lg:min-h-[600px] object-cover border border-gray-100"
              />
            </motion.div>
      ...
      {/* ── FEATURES ── */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-2">
        <div className="text-center mb-12 md:mb-16">
          <span className="text-[#49BCE3] font-semibold uppercase tracking-wider text-sm">
            Key Benefits
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-4xl font-extrabold text-slate-900 mt-3 mb-4">
            {city
      ...
      {service.sections && service.sections.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20 bg-gray-50">
          <div className="space-y-16 md:space-y-20">
            {service.sections.map((section, idx) => (
              ...
              >
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-6 border-l-4 border-[#49BCE3] pl-5">
                  {section.heading}
                </h2>
                <div className="text-gray-700 text-[15px] sm:text-base md:text-lg leading-relaxed space-y-6 max-w-4xl">
      ...
      {/* ── CTA SECTION ── */}
      <section className="bg-gradient-to-r from-[#0f2a5f] via-[#13387a] to-[#0f2a5f] py-16 md:py-12 relative overflow-hidden">
        <div className="absolute inset-0 bg-[#49BCE3]/10 blur-3xl opacity-30" />
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <h2 className="text-3xl sm:text-4xl md:text-4xl font-extrabold text-white mb-5 tracking-tight">
          ...
          <p className="text-gray-200 text-base sm:text-lg max-w-2xl mx-auto mb-10 leading-relaxed px-4">
            {city
              ? `Let's talk about your goals and build a strategy that delivers real, measurable results for your business in ${city.name}.`
              : "Let's talk about your goals and build a strategy that delivers real, measurable results for your business."}
          </p>

          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 px-4">
            <Link
              href="/contact"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-[#49BCE3] hover:bg-[#3aa6cc] text-white px-8 py-2 rounded-xl font-semibold text-base transition-all shadow-lg hover:shadow-xl hover:scale-105 active:scale-95"
            >
              {city ? `Talk to Our Team in ${city.name}` : "Talk to Our Team"}
      ...
      {/* ── TOP SECTION ── */}
      <section className="py-12 md:py-16 lg:py-14 bg-white">
        <div className="max-w-7xl mx-auto px-6 md:px-8 lg:px-10">
          <div className="max-w-7xl">
            {(service.aboutSection?.topSection?.sections || []).map(
              (item, i) => (
                <div key={i} className="mb-10">
                  <h2 className="text-xl sm:text-2xl md:text-2xl font-bold text-[#1a1a1a] mb-3 leading-snug">
                    {item.heading}
                  </h2>
                  <p className="text-[#3D3D3D] text-[15px] sm:text-base md:text-[16px] leading-relaxed font-normal">
                    {item.paragraph}
                  </p>
                </div>
              ),
      ...
      <section className="py-12 md:py-16 bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mb-3">
            {service.title} Services by City
          </h2>
          <p className="text-gray-500 text-sm mb-8">
            We provide {service.title} services across NCR:
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            {CITIES.map((c) => {
              ...
              return isActive ? (
                <span
                  key={c.slug}
                  className="inline-flex items-center gap-1.5 bg-[#49BCE3] text-white px-5 py-2.5 rounded-full text-sm font-semibold shadow-sm"
                >
                  <MapPin size={13} />
                  {c.name}
                </span>
          ...
          {/* City page par — main service page link */}
          {city && (
            <div className="mt-8">
              <Link
